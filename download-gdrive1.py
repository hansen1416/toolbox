...
try:
    ...
    parser = argparse.ArgumentParser(
        description="Google Drive folder and files download"
    )
    parser.add_argument(
        "-f",
        "--from",
    ...
    parser.add_argument(
        ...
        dest="dir_to",
        type=str,
        help="Initial local folder to receive tree and files",
    )
    parser.add_argument(
        "-l",
        "--lista",
    ...
    parser.add_argument(
        "-v", "--verbose", help="Writes what it´s doing.", action="store_true"
    )
...
def downloadFile(service, spaces, file_name, file_id, mimeType, dest_folder):
    ...
    if args.lista:
        ...
        if "application/vnd.google-apps" in mimeType:
            ...
            if "form" in mimeType:
                print(
                    "Google app Form: {} - cannot be downloaded. Skiping...".format(
                        file_name
                    )
                )
                valid = False
            ...
            with open(filepath, "wb") as wer:
                ...
                while done is False:
                    ...
                    if status is not None and (
                        status.total_size is not None
                        and status.resumable_progress is not None
                    ):
                        print(
                            "\rDownload %s (%s/%s): %d%%."
                            % (
                                file_name,
                                sizeof_fmt(status.total_size),
                                sizeof_fmt(status.resumable_progress),
...
def getFolderFiles(service, folderId, folderName, dest_folder, depth):
    ...
    for f in entries["files"]:
        ...
        try:
            ...
            print(
                "{} -ID: {} NAME: {} TYPE: {} -- FAILED ({}) -- Stored for later try".format(
                    spaces, f["id"], f["name"], f["mimeType"], e
                )
            )
...
def main(basedir):
    ...
    try:
        ...
        for from_dir in from_dirs:
            print(
                "Downloading folder and files from: {} -> to: {}".format(
                    from_dir, to_dir
                )
            )
            ...
            if args.lista:
                ...
                if not folderId is None:
                    ...
                    if num_skiped > 0:
                        print(
                            "{} total skiped files, not downloaded.".format(num_skiped)
                        )

                    print("Retrying - Files marked as Errored")
                    for item in toRetry:
                        print(
                            "{} -ID: {} NAME: {} TYPE: {} PATH: {}".format(
                                "", item[0], item[1], item[2], item[3]
                            )
                        )
                        try:
                            downloadFile(
                                service, "", item[0], item[1], item[2], item[3]
                            )
                        except Exception:
                            print(
...
if __name__ == "__main__":
    ...
    # "https://drive.google.com/drive/folders/1Tquahp2HWBP_R2tNi5cxsVJ3oEJ8F0Xx"
